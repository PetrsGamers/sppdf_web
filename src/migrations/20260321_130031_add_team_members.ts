...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(sql`CREATE TABLE \`landing_page_ribbon_items\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  	\`text\` text NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`landing_page_ribbon_items_order_idx\` ON \`landing_page_ribbon_items\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`landing_page_ribbon_items_parent_id_idx\` ON \`landing_page_ribbon_items\` (\`_parent_id\`);`)
  await db.run(sql`CREATE TABLE \`landing_page_divisions_items\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  	\`icon\` text NOT NULL,
  	...
  	\`description\` text,
  	\`image_id\` integer,
  	\`link_label\` text DEFAULT 'Zjistit více',
  	\`link_url\` text DEFAULT '#',
  	FOREIGN KEY (\`image_id\`) REFERENCES \`media\`(\`id\`) ON UPDATE no action ON DELETE set null,
  	FOREIGN KEY (\`_parent_id\`) REFERENCES \`landing_page\`(\`id\`) ON UPDATE no action ON DELETE cascade
  );
  `)
  await db.run(sql`CREATE INDEX \`landing_page_divisions_items_order_idx\` ON \`landing_page_divisions_items\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`landing_page_divisions_items_parent_id_idx\` ON \`landing_page_divisions_items\` (\`_parent_id\`);`)
  await db.run(sql`CREATE INDEX \`landing_page_divisions_items_image_idx\` ON \`landing_page_divisions_items\` (\`image_id\`);`)
  await db.run(sql`CREATE TABLE \`landing_page_footer_social_links\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  	\`icon\` text NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`landing_page_footer_social_links_order_idx\` ON \`landing_page_footer_social_links\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`landing_page_footer_social_links_parent_id_idx\` ON \`landing_page_footer_social_links\` (\`_parent_id\`);`)
  await db.run(sql`CREATE TABLE \`landing_page_footer_link_columns_links\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` text NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  	\`label\` text NOT NULL,
  	\`url\` text NOT NULL,
  	FOREIGN KEY (\`_parent_id\`) REFERENCES \`landing_page_footer_link_columns\`(\`id\`) ON UPDATE no action ON DELETE cascade
  );
  `)
  await db.run(sql`CREATE INDEX \`landing_page_footer_link_columns_links_order_idx\` ON \`landing_page_footer_link_columns_links\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`landing_page_footer_link_columns_links_parent_id_idx\` ON \`landing_page_footer_link_columns_links\` (\`_parent_id\`);`)
  await db.run(sql`CREATE TABLE \`landing_page_footer_link_columns\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`landing_page_footer_link_columns_order_idx\` ON \`landing_page_footer_link_columns\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`landing_page_footer_link_columns_parent_id_idx\` ON \`landing_page_footer_link_columns\` (\`_parent_id\`);`)
  await db.run(sql`CREATE TABLE \`landing_page\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`nav_cta_label\` text DEFAULT 'Přidat se',
  	\`nav_cta_link\` text DEFAULT '#',
  	\`hero_badge\` text DEFAULT 'Spolek přátel pedagogické fakulty',
  	\`hero_headline\` text NOT NULL,
  	\`hero_subtitle\` text,
  	\`hero_cta_primary_label\` text DEFAULT 'Zapojte se do spolku',
  	\`hero_cta_primary_link\` text DEFAULT '#',
  	\`hero_cta_secondary_label\` text DEFAULT 'Naše vize',
  	\`hero_cta_secondary_link\` text DEFAULT '#',
  	\`hero_image_id\` integer,
  	\`divisions_heading\` text DEFAULT 'Sekce našeho spolku',
  	\`divisions_description\` text DEFAULT 'Vyberte si oblast, která vás zajímá, a staňte se součástí týmu SPPDF.',
  	\`footer_description\` text DEFAULT 'Spolek přátel pedagogické fakulty je nezávislá organizace propojující generace učitelů a studentů v srdci naší univerzity.',
  	\`footer_copyright\` text DEFAULT '© 2024 Spolek přátel pedagogické fakulty. Všechna práva vyhrazena.',
  	\`updated_at\` text,
  	\`created_at\` text,
  	FOREIGN KEY (\`hero_image_id\`) REFERENCES \`media\`(\`id\`) ON UPDATE no action ON DELETE set null
  );
  `)
  await db.run(sql`CREATE INDEX \`landing_page_hero_hero_image_idx\` ON \`landing_page\` (\`hero_image_id\`);`)
}
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  await db.run(sql`DROP TABLE \`landing_page_ribbon_items\`;`)
  await db.run(sql`DROP TABLE \`landing_page_divisions_items\`;`)
  await db.run(sql`DROP TABLE \`landing_page_footer_social_links\`;`)
  await db.run(sql`DROP TABLE \`landing_page_footer_link_columns_links\`;`)
  await db.run(sql`DROP TABLE \`landing_page_footer_link_columns\`;`)
